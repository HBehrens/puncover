            ...
            return sum(
                [
                    ...
                    for s in itertools.chain(
                        s[collector.SUB_FOLDERS], s[collector.FILES]
                    )
                ]
            )
...
def style_background_bar_filter(context, x, total, color=None):
    ...
    x = min(x, total)
    percent = 100 * x // total
    return "background:linear-gradient(90deg, {1} {0}%, transparent {0}%);".format(
        percent, color
    )
...
def sorted_filter(context, symbols):
    ...
    key = {
        "name": lambda e: e.get(
            collector.DISPLAY_NAME, e.get(collector.NAME, None)
        ).lower(),
...
class HTMLRenderer(View):
    ...
    def url_for_symbol(self, value):
        if value[collector.TYPE] in [collector.TYPE_FUNCTION]:
            return self.url_for(
                "path", path=self.collector.qualified_symbol_name(value)
            )
...
class PathRenderer(HTMLRenderer):
    def dispatch_request(self, path=None):
        ...
        print("### " + path)
        for f in sorted(
            [f[collector.PATH] for f in self.collector.file_elements.values()]
        ):
            print(f)
        # print "## root folders"
...
class SymbolRenderer(HTMLRenderer):
    def dispatch_request(self, symbol_name=None):
        ...
        return redirect(
            self.url_for("path", path=self.collector.qualified_symbol_name(symbol))
        )
...
def register_urls(app, collector):
    app.add_url_rule(
        "/", view_func=OverviewRenderer.as_view("overview", collector=collector)
    )
    app.add_url_rule(
        "/all/", view_func=AllSymbolsRenderer.as_view("all", collector=collector)
    )
    app.add_url_rule(
        "/path/<path:path>/",
        view_func=PathRenderer.as_view("path", collector=collector),
    )
    app.add_url_rule(
